  ...
  return (
    <div className="flex gap-3 overflow-x-auto pb-1">
      {items.map((item) => (
        <GlassCard
          key={item.id}
          className="relative aspect-[9/16] w-24 shrink-0 overflow-hidden p-0"
        >
          ...
          {item.signedUrl && (
            <button
              ...
              className="absolute left-1 top-1 rounded-full bg-black/50 p-1.5 backdrop-blur-sm active:scale-95"
              aria-label="Baixar"
            >
          ...
          <button
            ...
            className="absolute right-1 top-1 rounded-full bg-black/50 p-1.5 backdrop-blur-sm active:scale-95"
            aria-label="Excluir"
          >
