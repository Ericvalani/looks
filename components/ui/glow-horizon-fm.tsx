...
export default function GlowHorizonFM({ className, variant = "top" }: GlowHorizonProps) {
  ...
  return (
    <motion.div
      ...
      // NOTE: only `transform` + `opacity` are animated so the whole thing runs
      // on the GPU compositor (smooth even while the page hydrates). Animating
      // `filter: blur()` here re-rasterizes the full viewport every frame and
      // was the source of the 2fps jank, so it's intentionally omitted.
      style={{ isolation: "isolate", willChange: "transform, opacity" }}
      initial={{ [axis]: enterPct, [scaleAxis]: 1.5, opacity: 0 }}
      animate={{ [axis]: restPct, [scaleAxis]: 1, opacity: 1 }}
  ...
  return (
    <motion.div
      ...
      style={{
        scale,
        background: color,
        // pre-rasterize the (static) blur once and only composite the transform
